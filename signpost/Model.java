...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int i = 0; i < solution.length; i++) {
            for (int j = 0; j < solution[i].length; j++) {
                int elem = solution[i][j];
                if (elem == 1 || elem == size()) {
                    Sq s = new Sq(i, j, elem, true, arrowDirection(i, j), 0);
                    _board[i][j] = s;
                } else {
                    Sq s = new Sq(i, j, 0, false, arrowDirection(i, j), -1);
                    _board[i][j] = s;
                }
                numArray.remove((Integer) elem);
            }
        }
        for (Sq[] col: _board) {
            for (Sq sq : col) {
        ...
        for (Sq[] row: _board) {
            for (Sq s: row) {
                PlaceList[][][] p = Place.successorCells(width(), height());
                s._successors = p[s.x][s.y][s.direction()];
                s._predecessors = new PlaceList();
            }
        }

        for (Sq[] row: _board) {
            for (Sq elem: row) {
                for (Place p: elem.successors()) {
                    get(p)._predecessors.add(elem.pl);
                }
            }
        }
    ...
    Model(Model model) {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y  = 0; y < _height; y++) {
                Sq s = model._board[x][y];
                int seqNum = s.sequenceNum(), dir = s.direction();
                int group = s.group();
                boolean hasFixed = s.hasFixedNum();
                _board[x][y] =
                        new Sq(x, y, seqNum, hasFixed, dir, group);
            }
        }
        for (Sq[] col: _board) {
            for (Sq sq : col) {
        ...
        for (int x = 0; x < width(); x++) {
            for (int y  = 0; y < height(); y++) {
                ...
                _board[x][y]._successor =
                        s._successor != null ? get(s.successor().x,
                                s.successor().y) : null;
                _board[x][y]._predecessor =
                        s._predecessor != null ? get(s.predecessor().x,
                                s.predecessor().y) : null;
                _board[x][y]._head = get(s.head().x, s.head().y);
    ...
    boolean autoconnect() {
        ...
        for (Sq start: _allSquares) {
            for (Sq finish: _allSquares) {
                if (start.sequenceNum() + 1 == finish.sequenceNum()
                        && start.connectable(finish)) {
            ...
            for (int y1 = 0; y1 < height(); y1++) {
                for (int x2 = 0; x2 < width(); x2++) {
                    for (int y2 = 0; y2 < height(); y2++) {
                        if (_solution[x1][y1] + 1 == _solution[x2][y2]) {
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            boolean correctDir, correctOrder, correctSeq, seperateSeq;
            correctDir =  Place.dirOf(this.x, this.y, s1.x, s1.y) == _dir;
            correctOrder = s1.predecessor() == null && this.successor() == null
                    && s1.sequenceNum() != 1
                    && this.sequenceNum() != width() * height();
            correctSeq = s1.sequenceNum() != 0 && this.sequenceNum() != 0
                    ? this.sequenceNum() == s1.sequenceNum() - 1 : true;
            seperateSeq = s1.sequenceNum() == 0 && this.sequenceNum() == 0
                    ? !this.head().equals(s1.head()) : true;
            return correctDir && correctOrder && correctSeq && seperateSeq;
        ...
        boolean connect(Sq s1) {
            ...
            int s0SeqNum = this.sequenceNum();
            int group = s1.group();
            int group2 = this.group();
            _unconnected -= 1;
            this._successor = s1;
            ...
            if (this.sequenceNum() != 0) {
                ...
                while (successor != null) {
                    successor._sequenceNum =
                            successor.predecessor().sequenceNum() + 1;
            ...
            if (s1.sequenceNum() != 0) {
                ...
                while (predecessor != null) {
                    predecessor._sequenceNum =
                            predecessor.successor().sequenceNum() - 1;
                    predecessor = predecessor.predecessor();
                }
            }
            Sq successor2 = s1;
            while (successor2 != null) {
                successor2._head = successor2.predecessor().head();
                successor2 = successor2.successor();
            }
            Sq prev = (s0SeqNum == 0  && this.sequenceNum() != 0) ? this : null;
            Sq prev1 = (s1SeqNum == 0  && s1.sequenceNum() != 0) ? s1 : null;
            ...
            if (this.sequenceNum() == 0 && s1.sequenceNum() == 0) {
                this.head()._group = joinGroups(this.group(), group);
            }
            return true;
        }

        /**
         * @return whether it is a single element or not
         */
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (isSingleElement() && next.isSingleElement()) {
                    releaseGroup(group());
                    this._group = next._group = -1;
                } else if (isSingleElement() && !next.isSingleElement()) {
                    next._group = _group;
                    _group = -1;
                } else if (!isSingleElement() && next.isSingleElement()) {
                    next._group = -1;
                ...
                createNewGroupPred();
                createNewGroupSucc(next);
            }
            for (Sq q = next; q != null; q = q.successor()) {
                q._head = next;
            }
        }
        /** Creates a new group for the pred, if needed. */
        void createNewGroupPred() {
            boolean hasFixed = false;
            for (Sq q = this; q != null; q = q.predecessor()) {
                if (q.hasFixedNum()) {
                    hasFixed = true;
                    break;
                }
            ...
            if (!hasFixed) {
                boolean hasPred = (this.predecessor() != null);
                int group = -1;
                if (hasPred) {
                    group = newGroup();
                }
        ...
        /**
         * @param next the next sq after this
         */
        void createNewGroupSucc(Sq next) {
            boolean hasFixed = false;
            for (Sq q = next; q != null; q = q.successor()) {
                if (q.hasFixedNum()) {
                    hasFixed = true;
            ...
            if (!hasFixed) {
                boolean hasSuccessor = (next.successor() != null);
                int group = -1;
                if (hasSuccessor) {
                    group = newGroup();
                }
